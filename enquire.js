...
export const ask = (message) => {
  if (typeof message !== "string") {
    throw TypeError('The "message" must be of type string.');
  }
  render.startSection();
  printf(
    " %s ".style(["bg-grey", "#000000", "bold"]).concat("".style("grey"), " "),
    message,
  );
...
export const confirm = (statement) => {
  if (typeof statement !== "string") {
    throw TypeError('The "statement" must be of type string.');
  }
  let choice;
  render.startSection();
  while (true) {
    printf(
      " %s (y/n)".style(["bg-grey", "#000000", "bold"]).concat(
        "".style("grey"),
        " ",
      ),
      statement,
...
export const secret = (message) => {
  ...
  render.startSection();
  printf(
    " %s: ".style(["bg-grey", "#000000", "bold"]).concat(
      "".style("grey"),
      " ",
    ),
    message,
...
export const choose = (options) => {
  if (!options || !Array.isArray(options)) {
    throw TypeError("Expected an argument of type String[]");
  }
  ...
  const renderUi = () => {
    let lines = options.map((opt, i) => {
      ...
      return i === index ? `◉ ${line}`.style(["bold"]) : `○ ${line}`;
    });
...
export const search = (options) => {
  if (!options || !Array.isArray(options) || options.length === 0) {
    throw TypeError(
      `Expected an argument of type String[], but got\n${stringify(options)}`,
    );
  }
  printf("%s", cursorHide);
  ttySetRaw();
  ...
  const renderUi = () => {
    ...
    const queryDisplay = query ||
      "Type to start search...".style(["grey", "italic"]);
    const queryDisplayWidth = queryDisplay.stripStyle().length;
    const horizontalGap = terminalWidth - 8 - queryDisplayWidth;

    if (horizontalGap > 0) {
      lines.push(
        queryDisplay.padEnd(queryDisplay.length + horizontalGap).border(
          "rounded",
        ),
      );
    } else {
      lines.push(
        queryDisplay.stripStyle().chunks(terminalWidth - 8).join("\n").border(
          "rounded",
        ),
      );
    }
    ...
    if (filtered.length === 0) {
      ...
      filtered.forEach((opt, i) => {
        ...
        if (horizontalGap < 0) {
          optionLines.push(
            lineDisplay.chunks(terminalWidth - 4)
...
export const select = (options) => {
  ...
  const renderUi = () => {
    ...
    lines.push(
      " Select one or more (Space to toggle, Enter to confirm) ".style([
        "#000000",
        "bold",
        "bg-grey",
      ]),
...
export const pick = () => {
  ...
  const renderUi = () => {
    const lines = [
      ...
      ...getDetails(),
      " ",
      " Select one (Enter to confirm, Arrow to navigate) ".style([
        "#000000",
        "bold",
        "bg-grey",
      ]),
...
export const describe = (buffer = "") => {
  ...
  const HELP_TEXT = " Textarea (press CTRL+D to submit) ".style([
    "#000000",
    "bold",
    "bg-grey",
  ]);
  const HELP_PADDED = HELP_TEXT.padStart(
    HELP_TEXT.stripStyle().length + terminalWidth - 12,
  );
...
export const edit = (content = "") => {
  ...
  if (!file) {
    throw Error(
      "Failed to open temp file for the editor. " + String(file) + filePath,
    );
  }
