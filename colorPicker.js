...
const getColoredBorderChar = (char) => {
  const { r, g, b } = hsvToRgb(
    state.selectedColor.h,
    state.selectedColor.s,
    state.selectedColor.v,
  );
...
export function colorPicker() {
  ...
  const focusNextSection = () => {
    switch (state.currentFocus) {
      case SELECTED_SECTION.hues:
        state.currentFocus = SELECTED_SECTION.saturations;
        break;
      case SELECTED_SECTION.saturations:
        state.currentFocus = SELECTED_SECTION.values;
        break;
      case SELECTED_SECTION.values:
        state.currentFocus = SELECTED_SECTION.result;
        break;
      case SELECTED_SECTION.result:
        state.currentFocus = SELECTED_SECTION.hues;
        break;
    }
    renderUI();
  };
  ...
  const focusNextHueValue = () =>
    state.selectedColor.h = (state.selectedColor.h + 1) % 360;
  const focusPrevHueValue = () =>
    state.selectedColor.h = (state.selectedColor.h + 359) % 360;

  const focusNextSaturationValue = () =>
    state.selectedColor.s = (state.selectedColor.s + 1) % 101;
  const focusPrevSaturationValue = () =>
    state.selectedColor.s = (state.selectedColor.s + 100) % 101;

  const focusNextValueValue = () =>
    state.selectedColor.v = (state.selectedColor.v + 1) % 101;
  const focusPrevValueValue = () =>
    state.selectedColor.v = (state.selectedColor.v + 100) % 101;
  ...
  const focusNextColorValue = () => {
    switch (state.currentFocus) {
      case SELECTED_SECTION.hues:
        ...
        break;
      case SELECTED_SECTION.saturations:
        focusNextSaturationValue();
        break;
      case SELECTED_SECTION.values:
        focusNextValueValue();
        break;
      case SELECTED_SECTION.result:
        focusNextResultValue();
        break;
    }
    renderUI();
  };
  ...
  const focusPrevColorValue = () => {
    switch (state.currentFocus) {
      case SELECTED_SECTION.hues:
        ...
        break;
      case SELECTED_SECTION.saturations:
        focusPrevSaturationValue();
        break;
      case SELECTED_SECTION.values:
        focusPrevValueValue();
        break;
      case SELECTED_SECTION.result:
        focusPrevResultValue();
        break;
    }
    renderUI();
  };
...
function renderUI() {
  ...
  printf(bottomBorder);
  const totalContentHeight = huesSection.length + colorSection.length +
    resultSection.length + keybindSection.length;
  prevCursorPos = cursorUp(totalContentHeight) +
    cursorBackward(terminalWidth + 1);
}
...
const generateKeybindView = () => {
  ...
  const renderCurrentLine = () => {
    ...
    const padding = " ".repeat(
      Math.max(0, Math.floor((terminalWidth - 2 - visibleLength) / 2)),
    );
...
function createBorders(borderStyle, contentWidth) {
  ...
  const bottomBorder = ansi.style.reset + borderStyle.bl + horizontalBorder +
    borderStyle.br + "\n";
...
function generateHues() {
  const currentBorder = SELECTED_SECTION.hues === state.currentFocus
    ? BORDER_CHARS.double
  ...
  while (hueCount < 360) {
    ...
    for (
      let hue = startOffset;
      hue < 360;
      hue += hueStep, hueCount++, currentLineLength++
    ) {
  ...
  const { topBorder, bottomBorder } = createBorders(
    currentBorder,
    maxLineLength + 3,
  );
...
function generateColorRange(type, viewWidth) {
  ...
  const currentBorder = state.currentFocus === type
    ? BORDER_CHARS.double
    : BORDER_CHARS.rounded;
...
function createColorValueBox(label, value) {
  return (addCustomeBorder(
    ` ${label}:${value} `,
    state.selectedMode === label ? BORDER_CHARS.double : BORDER_CHARS.rounded,
  )).split("\n");
}
...
function generateResultsView() {
  ...
  const hsvBox = createColorValueBox(
    SELECTED_MODE.hsv,
    `${parseInt(h)},${parseInt(s)},${parseInt(v)}`,
  );
  const rgbBox = createColorValueBox(
    SELECTED_MODE.rgb,
  ...
  const totalBoxWidth = hsvBox[0].stripStyle().length +
    rgbBox[0].stripStyle().length + hexBox[0].stripStyle().length +
    hslBox[0].stripStyle().length;
  ...
  for (let i = 0; i < 3; i++) {
    const colorBar = ansi.rgb(parseInt(r), parseInt(g), parseInt(b)) +
      ...
      " " + ansi.style.reset;
    displayLines.push(
      ` ${rgbBox[i]} ${hsvBox[i]} ${hexBox[i]} ${hslBox[i]} ${colorBar}`,
    );
  }
...
function displayColorSections() {
  ...
  const combinedWidth = saturationLines[0].stripStyle().length +
    valueLines[0].stripStyle().length;
